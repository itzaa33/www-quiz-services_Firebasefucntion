...
export const getQuizsTitle = functions.https.onRequest((request, response) => {

    // var datetime = admin.firestore.FieldValue.serverTimestamp();

    cors(request, response, async () => {

        const obj = await getQuizdata()
...
async function getQuizdata() {  
    ...
    try {
        ...
        const data = await admin.firestore().collection('quizzes').where('Time_Start', "<=", datetime).get()
        if( data !== null)
        {
            data.forEach(doc => {
...
export const getQuizsform = functions.https.onRequest((request, response) => {
    ...
    cors(request, response, async () => {
        ...
        try {
            ...
            const items = await getQuizdata()

            const id = items[0].id

            let data = await admin.firestore().collection('quizzes/' + id + '/questions').get()
...
export const post_calculate_point = functions.https.onRequest((request, response) => {
    ...
    cors(request, response, async () => {
        ...
        const id_user = await checkAuth_user(form.idToken_user)
        ...
        if (id_user !== null) 
        {
            if (form_quiz.length >= 0 && form_quiz.length <= 60) 
            {
                ...
                .then(async  point_return => 
                {

                    await add_pointUser(form.id_quiz, id_user, point_return, form.datetime, true_quiz, false_quiz)
                    await update_ranking(form.id_quiz, id_user, point_return, form.datetime, user_data, 1, true_quiz, false_quiz)
                    response.end();
            ...
            else
            {
              ...
              await  add_pointUser(form.id_quiz, id_user, point, form.datetime, true_quiz, false_quiz)
              await  update_ranking(form.id_quiz, id_user, point, form.datetime, user_data, 1, true_quiz, false_quiz)
              response.end();
...
export const Update_User = functions.https.onRequest((request, response) => {
    ...
    cors(request, response, async () => {
        ...
        try {
            ...
            const id_quiz = await getQuizdata()

            if(id_quiz !== undefined)
            {
                await update_ranking(id_quiz[0].id, user_data.uid, 0, new Date(), user_data, 0, 0, 0)
            }
...
async function checkAuth_user(idToken_user) {


    try {
        const decodedToken = await admin.auth().verifyIdToken(idToken_user);
...
async function add_pointUser(id_quiz, id_user, point, datetime, true_quiz, false_quiz) {
    ...
    try {
        if (id_quiz !== null && id_quiz !== undefined) {
            ...
            await admin.firestore().collection('quizzes/' + id_quiz + '/history_point').add(setData)
...
async function update_ranking(id_quiz, id_user, point, datetime, user_data, valueForm, true_quiz, false_quiz) {

    try {
        var form_rank = await admin.firestore().collection('quizzes/' + id_quiz + '/ranking').doc(id_user).get()
    ...
    if (form_rank.data() !== undefined) {
        ...
        if (form_rank.get('point') < parseInt(point) && valueForm !== 0) {
            ...
            if(true_quiz === 0 && false_quiz === 0)
            {
                await admin.firestore().doc('quizzes/' + id_quiz + '/ranking/' + id_user).update({
        ...
        await admin.firestore().collection('quizzes/' + id_quiz + '/ranking/').doc(id_user).set({
...
export const getHigthscoreMe = functions.https.onRequest((request, response) => {
        ...
        try {
            ...
            .then(arr => {
                ...
                res = {
                    ...
                    // statusImage:form_user.statusImage   || false,
                }   
...
export const getHigthscore = functions.https.onRequest((request, response) => {
...
async function getQuizdataBefore() {  
    var datetime = new Date();
    ...
    try {
        ...
        const data = await admin.firestore().collection('quizzes').orderBy('Time_End','desc').get()
        if( data !== null)
        {
            data.forEach(doc => {
                if (doc.get('Time_End') <= datetime) {
...
export const getHigthscoreBefore = functions.https.onRequest((request, response) => {
    ...
    cors(request, response, async () => {
        ...
        try {
            ...
            const items = await getQuizdataBefore()
    ...
    cors(request, response, async () => {
        ...
        try {
            ...
            await axios.post(url,data,
                            {headers: { Authorization: "Bearer 387740040|oTvIdcguuU" }},
